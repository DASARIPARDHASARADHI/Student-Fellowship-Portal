...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_sql = "UPDATE students SET appr_by_dept_offc = ?, dept_offc_remarks = ? WHERE rollno = ?";
    $stmt_update = $conn->prepare($update_sql);
    $stmt_update->bind_param("sss", $approval_status, $dept_offc_remarks, $rollno);
...
<body>
    ...
    <div class="container">
            ...
            <div class="form-group">
                ...
                <input type="text" name="claimed_month" id="claimed_month" value="<?php
                                                                                    ...
                                                                                    if (!empty($student['claimed_month'])) {
                                                                                        ...
                                                                                        echo $date->format('F Y'); // 'F' for full month name, 'Y' for 4-digit year
                                                                                    } else {
        ...
        <form action="dept_offc_approve_indv.php" method="post">
            <input type="hidden" name="rollno" value="<?php echo $student['rollno']; ?>">
            <div class="form-group">
                <label for="absent_days">Number of Absent Days:</label>
                <input type="number" name="absent_days" id="absent_days" min="0" max="31" oninput="validateAbsentDays()">
            </div>

            <script>
                function validateAbsentDays() {
                    const absentDaysInput = document.getElementById("absent_days");
                    let absentDaysValue = parseInt(absentDaysInput.value, 10);

                    // Check if the value is outside the allowed range
                    if (absentDaysValue < 0 || absentDaysValue > 31) {
                        alert("Please enter a number between 0 and 31.");
                        absentDaysInput.value = ''; // Clear the input if out of range
                    }
                }
            </script>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select name="approval_status" id="approval_status">
                    <option value="YES" <?php if ($student['appr_by_dept_offc'] == 'YES') echo 'selected'; ?>>Approve</option>
                    <option value="REJECTED" <?php if ($student['appr_by_dept_offc'] == 'REJECTED') echo 'selected'; ?>>Reject</option>
                </select>
            </div>
            <div class="form-group">
                <label for="dept_offc_remarks">Remarks:</label>
                <textarea name="dept_offc_remarks" id="dept_offc_remarks" placeholder="Enter your remarks" rows="5" cols="30"></textarea>
            </div>
        ...
        <div class="logout">
            <a href="dept_offc.php">Back to Home</a>
        </div>
    </div>

    <script>
        function calculateApprovedAmount() {
            const claimedAmount = parseFloat("<?php echo $student['claimed_amount']; ?>") || 0;
            const claimedMonth = "<?php echo $student['claimed_month']; ?>";
            const absentDays = parseInt(document.getElementById("absent_days").value) || 0;

            if (!claimedMonth) {
                document.getElementById("approved_amount").value = "No date provided";
                return;
            }

            const [year, month] = claimedMonth.split("-").map(Number);
            const daysInMonth = new Date(year, month, 0).getDate();

            // Calculate approved amount based on the formula
            const approvedAmount = claimedAmount - (claimedAmount / daysInMonth) * absentDays;
            document.getElementById("approved_amount").value = approvedAmount.toFixed(2);
        }

        document.getElementById("absent_days").addEventListener("input", calculateApprovedAmount);
    </script>
</body>
