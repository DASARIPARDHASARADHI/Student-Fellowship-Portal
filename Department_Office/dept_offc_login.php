...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($pwd === $row['dept_offc_password'] && $row['dept_offc'] == 'YES') {
            // Password is correct, start session and redirect to dept_offc.php
            session_start();
            $_SESSION['webmail'] = $row['webmail'];
            $_SESSION['name'] = $row['name']; // Assuming the employee name is stored
            header("Location: dept_offc.php");
            exit();
        } else {
...
<head>
    ...
    <title>Department Office Login</title>
...
<body>
    ...
    <h2 style="color: white; font-size: 25px;">Department Office Login</h2>

    <div id="container">
        <form id="registrationForm" action="dept_offc_login.php" method="POST">
