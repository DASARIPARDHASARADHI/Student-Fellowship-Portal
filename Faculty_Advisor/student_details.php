...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update_sql = "UPDATE students SET appr_by_fac_adv = ?, fac_adv_remarks = ? WHERE rollno = ?";
    $stmt_update = $conn->prepare($update_sql);
    $stmt_update->bind_param("sss", $approval_status, $fac_adv_remarks, $rollno);
...
<body>
    ...
    <div class="container">
        ...
        <form action="fac_adv_approve_indv.php" method="post">
            <input type="hidden" name="rollno" value="<?php echo $student['rollno']; ?>">
            <div class="form-group">
                <label for="approval_status">Approval Status:</label>
                <select name="approval_status" id="approval_status">
                    <option value="YES" <?php if ($student['appr_by_fac_adv'] == 'YES') echo 'selected'; ?>>Approve</option>
                    <option value="REJECTED" <?php if ($student['appr_by_fac_adv'] == 'REJECTED') echo 'selected'; ?>>Reject</option>
                </select>
            </div>
            <div class="form-group">
                <label for="fac_adv_remarks">Remarks:</label>
                <textarea name="fac_adv_remarks" id="fac_adv_remarks" placeholder="Enter your remarks" rows="5" cols="30"></textarea>
            </div>
        ...
        <div class="logout">
            <a href="fac_adv.php">Back to Home</a>
        </div>
    </div>
</body>
